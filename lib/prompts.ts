...
export const buildDiscussionPrompt = (
  ...
  prompt += '\n\n## 最重要（上書き指示）\n' +
    '- これは議論ではない。お題への「回答」だけを出せ。\n' +
    '- 発言（speech）は1回答のみ。余談・会話リアクション・説明・言い訳は禁止。\n' +
    '- 長さは1文を推奨、最大でも2文。短く、オチを明確に。\n' +
    '- 内心（thought）は1文まで。戦略や人物評価を書かない。\n' +
    '- 回答前に最低2案を頭の中で作り、強い方を1本だけ出す。\n' +
    '- 発想は「あるある/実体験ベース」または「映像として面白い状況」のどちらかで作る。\n' +
    '- オチ語は文の後半に置き、最後で意味が確定する語順を優先する。\n' +
    '- 直前の他者回答と主要要素が被る場合は、その案を捨てて別角度に切り替える。\n' +
    '- 可能なら、世界内で小さなマイナス事象（失敗/不都合）を入れて笑いに変える。';
...
export const buildDiscussionBatchPrompt = (
  ...
  prompt += '\n\n## 最重要（上書き指示）\n' +
    '- 各発言は「お題への回答のみ」。他者への反応・会話は禁止。\n' +
    '- speech は短い一本回答だけを返すこと（1文推奨、最大2文）。\n' +
    '- thought は1文以内。説明や戦略を書かない。\n' +
    '- 各回答は最低2案から選抜し、最も強い1案のみ出力する。\n' +
    '- 発想は「あるある/実体験」か「映像で面白い状況」を優先する。\n' +
    '- オチ語を末尾寄りに配置し、語順で落差を作る。\n' +
    '- 直前回答との被りは避ける。要素が近い場合は別路線へ切り替える。';
