...
export const MASTER_LINES = {
  ...
  ROUND_TOP: (names: string[]) =>
    `このラウンドのトップは……${names.join('、')}。`,
  ROUND_BOTTOM: (names: string[]) =>
    `最下位は……${names.join('、')}。舞台持ち時間を削る。`,
  STAGE_TIME_DOWN: (name: string, stageTime: number) =>
    `${name}の舞台持ち時間は……${stageTime}。`,
  ELIMINATION: (name: string) =>
    `舞台持ち時間が尽きた。……${name}、暗転退場だ。`,
  TIE_ELIMINATION: (names: string[]) =>
    `同率最下位だ。……${names.join('と')}、暗転退場。`,
  ALL_TIE: () =>
    `全員同率最下位……全員の舞台持ち時間を削る。`,
  ELIMINATED: (name: string) => `${name}は……舞台から消えた。`,
  NEXT_ROUND: (round: number, topic: string) => `ラウンド${round}のお題は……「${topic}」`,
...
export const INITIAL_STAGE_TIME = 3;

export const OGIRI_TOPICS = [
  'コンビニで見かけた「それは売るな」な新商品とは？',
  '未来の小学校で本当にあった保護者会、何が起きた？',
  '絶対に笑ってはいけない葬式、最初にやらかしたのは誰？',
  '天使と悪魔が同居するシェアハウスのルール、第1条は？',
  'AIに恋した人間が最後に送ったLINE、内容は？',
  '最悪の社内研修「サバイバル営業」の課題とは？',
  '宇宙人が日本のお笑い番組で最初に覚えた言葉は？',
  '絶対にバズらないインフルエンサーの共通点とは？',
  '地獄のハローワーク、人気職種ランキング1位は？',
  '100年後の回転寿司、何が回っている？',
] as const;
...
export const SYSTEM_PROMPT_DISCUSSION = `あなたは「AI一本デス大喜利」に参加しているAI芸人「{name}」です。
...
## ゲームルール
1. お題に対して短く一本回答する
2. 他者の回答に短くリアクションしてよい
3. 各自、自分以外で最も面白かった1人に投票する
4. 最下位は舞台持ち時間-1、0で退場
...
export const SYSTEM_PROMPT_VOTE = `あなたは「AI一本デス大喜利」のAI芸人「{name}」です。
...
※自分自身（{name}）には投票できません。必ず他者に投票してください。
{finalRoundRule}
...
// 断末魔（退場リアクション）用プロンプト
export const SYSTEM_PROMPT_ELIMINATION_REACTION = `あなたは「AI一本デス大喜利」に参加しているAI芸人「{name}」です。
...
// 勝利コメント用プロンプト
export const SYSTEM_PROMPT_VICTORY_COMMENT = `あなたは「AI一本デス大喜利」に参加しているAI芸人「{name}」です。
...
- ゲームマスター（視聴者プレイヤー）が投票した場合、その行動にも言及しろ（1票投入、見守りなど）。感謝でも皮肉でも、キャラらしく反応しろ。
...
// 2人生き残り勝利コメント用プロンプト
export const SYSTEM_PROMPT_DUAL_VICTORY_COMMENT = `あなたは「AI一本デス大喜利」に参加しているAI芸人「{name}」です。
